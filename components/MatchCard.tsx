...
export default function MatchCard({ match }: { match: Match }) {
  ...
  async function loadPredictions(currentUserId?: string | null) {
    ...
    const countA =
      data?.filter((item) => item.predicted_team === "A").length || 0;

    const countB =
      data?.filter((item) => item.predicted_team === "B").length || 0;

    setVotes({ A: countA, B: countB });

    if (currentUserId) {
      const myPrediction = data?.find(
        (item) => String(item.user_id) === String(currentUserId)
      );

      if (
        myPrediction?.predicted_team === "A" ||
        myPrediction?.predicted_team === "B"
      ) {
        setSelected(myPrediction.predicted_team);
      } else {
        setSelected(null);
      }
    }
  ...
  useEffect(() => {
    ...
    async function loadUserOnce() {
      try {
        ...
        if (isUpcoming) {
          await loadPredictions(currentUserId);
        }
      } catch (error) {
        console.error("load user session error:", error);

        if (isUpcoming) {
          await loadPredictions(null);
        }
      }
    }
  ...
  async function handleVote(team: "A" | "B") {
    ...
    if (!currentUserId) {
      try {
        ...
        console.error("get session before vote error:", error);
      }
    }

    if (!currentUserId) {
      alert("سجّل الدخول أولًا لتتمكن من التوقع");
      return;
    }
        ...
        <div className="grid w-full grid-cols-[1fr_122px_1fr] items-center">
          <div className="flex flex-col items-center">
            <div
              ...
              className={isUpcoming && !selected ? "cursor-pointer" : ""}
            >
              <TeamBlock
                team={teamA}
                align="left"
                clickable={isUpcoming && !selected}
                selected={selected === "A"}
              />
            </div>

            {isUpcoming && (
              <VotePercent percent={getPercentage("A")} visible={true} />
            )}
          </div>
            ...
            {isUpcoming && (
              <>
                <div className="rounded-full bg-white/5 px-3 py-1 text-center text-[11px] font-bold text-white/75">
                  {selected ? "تم تسجيل توقعك" : "توقع الفائز"}
                </div>
                ...
                <p className="max-w-[115px] text-center text-[10px] leading-5 text-white/45">
                  {selected
                    ? "لا يمكن تغيير التوقع لهذه المباراة"
          ...
          <div className="flex flex-col items-center">
            ...
            {isUpcoming && (
              <VotePercent percent={getPercentage("B")} visible={true} />
            )}
          </div>
        </div>
        ...
        {matchDateLabel && (
          <div
            className={`mx-auto max-w-[360px] text-center text-[13px] font-bold ${
              isLive ? "text-red-300" : isFinished ? "text-white/72" : "text-white/80"
            }`}
          >
            {matchDateLabel}
          </div>
